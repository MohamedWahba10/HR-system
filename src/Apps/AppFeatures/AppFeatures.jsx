...
export default function AppFeatures() {
    const [selectedItems, setSelectedItems] = useState({
      ...
      <Layout>
          ...
          <h1 className="text-2xl font-semibold mb-4 px-4">App Features</h1>
       <div className='py-2 mx-10'>
       <h2 className="text-lg text-black font-semibold mb-4">Choose what to show to employees</h2>
          {sections.map((section, index) => (
            <div key={index} className="mb-6">
              <h3 className="text-base font-medium mt-2">{section.title}</h3>
              <div className="grid grid-cols-2 gap-5 my-4">
                {section.items.map((item) => (
                  <label
                    key={item}
                    className={`flex items-center text-base justify-between border rounded-lg p-2 cursor-pointer ${
        ...
        <div className='flex justify-center items-center gap-2 my-2'>
          <button className='w-24 py-1 px-4 border border-primary bg-primary text-white rounded-lg'>Save</button>
          <button className='w-24 py-1 px-4 border border-primary text-primary rounded-lg'>Cancel</button>
